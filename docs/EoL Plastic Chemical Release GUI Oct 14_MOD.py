...
ASSUMED_VALUES={"Plastic waste lost to littering":0.02, "Plastic waste leak after landfill":0.1, "Plastic content in compost":0.01,
...
LOW_ADD_FRACTIONS = {"Plasticizer":0.1, "Flame Retardant":0.007, "UV Stabilizer": 0.005, "Heat Stabilizer":0.005, "Antioxidant":0.005, "Slip Agent":0.001, "Lubricant":0.001,
...
CONDITION_CATEGORIES = ["Total MSW (Tons):", "Total Plastic waste (Tons):", "Plastic Recycled (Total, domestic and export)",
...
WASTE_TYPES = ["Misc. Inorganic Waste", "Other", "Yard Trimmings", "Food", "Rubber, Leather and Textiles", "Wood", "Metals", "Glass",
                 "Paper and Paperboard", "Plastics"]

CALC_WASTE_TYPES = ["Misc. Inorganic Waste", "Other", "Yard Trimmings", "Food", "Rubber, Leather and Textiles", "Wood", "Metals", "Glass",
...
INTERNATIONAL_PLASTICS = ["Ethylene", "Vinyl Chloride", "Styrene", "Other"]

#Categories for life cycle inventory (formerly known as material flow analysis)
INVENTORY_CATEGORIES = ["PET", "HDPE", "PVC", "LDPE", "PLA", "PP", "PS", "Other Resin", "Chemical Additives"]

#Dictionary of densities of plastics for later calculations
DOMESTIC_PLASTICS_DENSITIES = {"PET":1.365, "HDPE":952.5, "PVC":1.455, "LDPE":0.925, "PLA":1.26, "PP":905, "PS":1.055, "Other Resin":1.29}
...
def additiveMassCalculator(additiveList, plasticType, massDict): #Takes argument of LIST of types of additives going into type of plastic, STRING of type of plastic, then DICT of bulk masses
    newDict = dict(zip(additiveList, [massDict[plasticType]*LOW_ADD_FRACTIONS[i] for i in additiveList])) #takes bulk mass and multiplies by low additive Fraction for each kind of additive
    return newDict
...
def backwardsLumpPlasticCalculator(resinMassList, typeOfResin, additiveList):
    additiveFraction = sum([LOW_ADD_FRACTIONS[i] for i in additiveList]) #Finds total Fraction of bulk mass that is resin
    lumpSum = resinMassList[typeOfResin]/(1-additiveFraction) #Divides to find bulk mass
    return lumpSum


def trvwListMaker(list_of_dicts): #Creates lists that will be eventually added to LCI TRVW tables. Takes argument of list of dictionaries that are to be examined
    new_list = []
    for i in INVENTORY_CATEGORIES: #iterates over list of categories in LCI tables
        sub_list = []
        q=0
        sub_list.append(i)
        for d in list_of_dicts:
            ...
            try:
                ...
                sub_list.append(d[i]) #if value is not a number (if it is 'Unavaible'), it will be added
        for b in range(len(sub_list)):
            if sub_list[b] == 0:
                sub_list[b] = "Negligible" #Changes 0's to negligible
        new_list.append(sub_list)
    return new_list

def streamSummaryTRVWLister(list_of_dicts, category): #creates lists that will be added to stream summary TRVW tables. Takes argument of list of dictionaries that are to be examined, along with string for category that will make up the row of the table
    trvw_list = []
    trvw_list.append(category) #adds category/row name
    for i in list_of_dicts:
        if category in i:
            trvw_list.append(i[category]) #adds values corresponding to category from dictionary to this list if there is one, otherwise adds 0
        else:
            trvw_list.append(0)
    return trvw_list
#Will accomplish recycling scaling calculations (Sensitivty Facts G9:G16)
def recycleScaler(reported_list, plastic_total, recycled_fraction): #takes input of types of plastics, total plastic mass, and Fraction of plastic that is recycled
    newScaledDict = dict(zip(typesOfPlasticDomestic,[plastic_total*recycled_fraction/sum(reported_list)*i for i in reported_list])) #creates dictionary from above list
...
def check_entry(check): #will be used to make sure all data has an input
    if check == []:
        return True

def make_calcs():
    assign_vals() #will be removed before distribution. This is a programming shortcut

    #creates list of lists of input data
    data_lists = [conditions, mswCompProp, mswRecyc, mswIncin, mswLand, mswCompost, repRecPlastics, repPlasticImport, repPlasticsExport,
    ...
    plastic_land_fractions = dict(zip(typesOfPlasticDomestic, plasticLandFractionsList))
    ...
    mswGeneratedProps = dict(zip(WASTE_TYPES, mswCompProp)) #key = MSW waste, value = proportion of MSW

    mswConditions = dict(zip(CONDITION_CATEGORIES, conditions)) #key = conditions (total MSW, total plastic, plastic recycled, etc.)


    #Creates dictionary of international recycling values
    repPlasticImportDict = dict(zip(INTERNATIONAL_PLASTICS, repPlasticImport))

    repPlasticsExportDict = dict(zip(INTERNATIONAL_PLASTICS, repPlasticsExport))

    repPlasticsReExportDict = dict(zip(INTERNATIONAL_PLASTICS, repPlasticsReExport))
    ...
    averageDensityCalculation = sum([DOMESTIC_PLASTICS_DENSITIES[i]*plasticFractionsRecycled[i] for i in typesOfPlasticDomestic])* 0.00000110231
    ##########################################################################################################################
    ...
    stream8MSWMasses_ = dict(zip(CALC_WASTE_TYPES, [mswCompProp[i]*conditions[0] for i in range(len(CALC_WASTE_TYPES))]))
    ...
    stream11MSWValues = dict(zip(CALC_WASTE_TYPES, [mswIncin[0]*mswIncin[i] for i in range(1,len(CALC_WASTE_TYPES)+1)]))
    ...
    stream12MSWValues = dict(zip(CALC_WASTE_TYPES, [mswLand[0]*mswLand[i] for i in range(1, len(CALC_WASTE_TYPES)+1)]))
    ...
    #MSW for this stream
    stream13MSW = dict(zip(CALC_WASTE_TYPES, [mswCompost[i+1]*mswCompost[0] for i in range(10)]))
    #######################################################################################################################
    ...
    stream14MSWValues = dict(zip(CALC_WASTE_TYPES, [mswRecyc[0]*mswRecyc[i] for i in range(1, len(CALC_WASTE_TYPES)+1)]))
    ...
    stream25ResinMasses = dict(zip(typesOfPlasticDomestic, [(stream24ResinMasses[i]+stream23ResinMasses_[i])*(1-ASSUMED_VALUES["Incineration Efficiency Fraction"]) for i in typesOfPlasticDomestic]))
    stream25AdditiveMasses = dict(zip(otherResinAdditives, [(stream24AdditiveTotals[i]+stream23AdditiveMasses_[i])*(1-ASSUMED_VALUES["Incineration Efficiency Fraction"]) for i in otherResinAdditives]))

    stream25MSWValues = dict(zip(CALC_WASTE_TYPES, [(stream11MSWValues[i])*(1-ASSUMED_VALUES["Incineration Efficiency Fraction"]) for i in CALC_WASTE_TYPES]))
    ...
    stream26PlasticMasses = dict(zip(typesOfPlasticDomestic, [stream26MassBasis*plastic_land_fractions[i] for i in typesOfPlasticDomestic]))
    ...
    stream29ResinMasses = dict(zip(typesOfPlasticDomestic, [stream4ResinMasses_[i] * ASSUMED_VALUES["Plastic waste leak after landfill"] for i in typesOfPlasticDomestic]))
    stream29AdditiveMasses = dict(zip(otherResinAdditives, [stream4AdditiveMasses_[i]*ASSUMED_VALUES["Plastic waste leak after landfill"]+(stream26AdditiveTotals[i]+stream23AdditiveMasses_[i])*0.00001 for i in otherResinAdditives]))
    ...
    matFlowManufactureLitter = dict(zip(INVENTORY_CATEGORIES, ["Unavailable" for i in INVENTORY_CATEGORIES]))
    ...
    #Emissions:
    matFlowCSPGHG = dict(zip(INVENTORY_CATEGORIES, [wasteFacilityEmissions/totalStream10Waste for i in INVENTORY_CATEGORIES]))
    ...
    matFlowMechRecycLittering = dict(zip(INVENTORY_CATEGORIES, [matFlowMechRecycInput[i]*0.0001 for i in INVENTORY_CATEGORIES]))

    #Inhalation Exposure (105/(9.072*10^8)*21834*250)/matFlowInputDivisor*Input
    matFlowMechRecycInhal = dict(zip(INVENTORY_CATEGORIES, [matFlowMechRecycInput[i]*(105/(9.072*10**8)*21834*250)/matFlowMechRecycInputDivisor for i in INVENTORY_CATEGORIES]))

    #Dermal Exposure (2170/(9.072*10^8))*21834*250*Input/matFlowInputDivisor
    matFlowMechRecycDermExp = dict(zip(INVENTORY_CATEGORIES, [matFlowMechRecycInput[i]*(2170/(9.072*10**8))*21834*250/matFlowMechRecycInputDivisor for i in INVENTORY_CATEGORIES]))
    ...
    #Output: 0
    matFlowIncinOutput = dict(zip(INVENTORY_CATEGORIES, [0 for i in INVENTORY_CATEGORIES]))
    ...
    matFlowIncinInhal = dict(zip(INVENTORY_CATEGORIES, [0 for i in INVENTORY_CATEGORIES]))
    ...
    matFlowLandGHG = dict(zip(INVENTORY_CATEGORIES, [0.04*1.10231 for i in INVENTORY_CATEGORIES]))
    ...
    streamTRVWLists = [streamSummaryTRVWLister(listOfStreamsForResinTRVW, i) for i in typesOfPlasticDomestic]+[streamSummaryTRVWLister(listOfStreamforAdditivesTRVW, i) for i in otherResinAdditives]+[streamSummaryTRVWLister(listOfStreamMSWTRVW, i) for i in CALC_WASTE_TYPES]
    ...
    listsWithoutTitles = [streamSummaryTRVWLister(listOfStreamsForResinTRVW, i) for i in typesOfPlasticDomestic]+[streamSummaryTRVWLister(listOfStreamforAdditivesTRVW, i) for i in otherResinAdditives]+[streamSummaryTRVWLister(listOfStreamMSWTRVW, i) for i in CALC_WASTE_TYPES]
  ...
  #PIE CHART
    ...
    ax1.pie(piecharttest, labels=WASTE_TYPES, explode=plasticexplode, autopct='%1.1f%%', pctdistance=0.9, labeldistance=1.05,
            shadow=True, startangle=180)
    ax1.set_title('MSW Composition', fontsize=18) #adjust the title of the figure. pad = distance from the figure
    ax1.plot(label=WASTE_TYPES)
...
mat_flow_manuf_trvw = ttk.Treeview(materialFlowFrame)
mat_flow_use_trvw = ttk.Treeview(materialFlowFrame)
mat_flow_csp_trvw = ttk.Treeview(materialFlowFrame)
mat_flow_mech_recyc_trvw = ttk.Treeview(materialFlowFrame)
mat_flow_incin_trvw = ttk.Treeview(materialFlowFrame)
mat_flow_land_trvw = ttk.Treeview(materialFlowFrame)
...
matFlowTRVWList = [matFlowManufactureText, mat_flow_manuf_trvw, matFlowUseText, mat_flow_use_trvw, matFlowCSPText, mat_flow_csp_trvw, matFlowMechRecycText,
                   mat_flow_mech_recyc_trvw, matFlowIncinText, mat_flow_incin_trvw, matFlowLandText, mat_flow_land_trvw]
...
def fillMatFlowAnalSumTRVW(): #will be used to fill each LCI trvw table
    ...
    for record in manufactureList:
        mat_flow_manuf_trvw.insert(parent ='', index ='end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4], record[5], record[6]))
        count +=1

    count = 0
    for record in useList:
        mat_flow_use_trvw.insert(parent ='', index ='end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4], record[5], record[6]))
        count +=1

    count = 0
    for record in cspList:
        mat_flow_csp_trvw.insert(parent ='', index ='end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4], record[5], record[6]))
        count +=1

    count = 0
    for record in mechRecycList:
        mat_flow_mech_recyc_trvw.insert(parent ='', index ='end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4], record[5], record[6]))
        count +=1

    count = 0
    for record in incinList:
        mat_flow_incin_trvw.insert(parent ='', index ='end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4], record[5], record[6]))
        count +=1

    count = 0
    for record in landList:
        mat_flow_land_trvw.insert(parent ='', index ='end', iid = count, text = '', values = (record[0], record[1], record[2], record[3], record[4], record[5], record[6]))
        count +=1
...
def checkProportions(listOfEntries, finalSum):
    ...
    for i in listOfEntries:
        try:
            ...
            gap_label_1.config(text = 'Error: Please enter a number into each box to continue.', fg = 'red')
            return
    if  0.99*finalSum<g<1.01*finalSum: #checks to be within 1%
        gap_label_1.config(text = 'Check successful.') #indicates success
        return True
    else:
        gap_label_1.config(text = 'Proportions do not sum to 1.', fg = 'red') #indicates failure
        return False
...
def conditionsCheckProp():
    ...
    for i in conditionsPropCheckList:
        try: #makes sure input is numbers only
            ...
            gap_label_1.config(text = 'Error: Please enter a number into each box to continue.')
            return
    ...
    if 0.99<g<1.01: #makes sure total waste proportions (incinerated, landfilled, recycled) sum to 1
        if 0.99*standard<h<1.01*standard:
            gap_label_1.config(text = 'Check successful.')
            return True
        else:
            gap_label_1.config(text = 'Domestic and exported recycling do not sum to total recycled Fraction.') #for if
            gap_label_1.config(fg = 'red')
    else:
        if 0.99*standard<h<1.01*standard:
            gap_label_1.config(fg = 'red')
            gap_label_1.config(text = "Plastic recycled proportion, landfill proportion, and incineration proportion do not sum to 1.")
        else:
            gap_label_1.config(text = "Plastic recycled proportion, landfill proportion, and incineration proportion do not sum to 1. Domestic and exported recycling do not sum to total recycled Fraction.")
            gap_label_1.config(fg = 'red')
...
def enter(entry, appList, valueLabel, nextLabel, nextEntry, nextCheck, nextAuto, nextEnter):
    ...
    for i in entry:
        try:
            ...
            gap_label_1.config(text = 'Error, please enter a number in each box.')
            return
        if float(entry[0].get()) >1: #this will create list that can be checked later on for correct proportions
            new_list = [entry[i] for i in range(1, len(entry))]
        else:
            new_list = entry
        ...
        if entry !=conditionsentryList and entry != RepRecycPlasticEntry and entry != ImportPlasticEntry and entry != ExportPlasticEntry and entry != ReExportPlasticEntry:
            if checkProportions(new_list, 1):
                appList.append(value) #if check is successful, will append data
            else:
                checkProportions(new_list, 1) #if check is unsuccessful, will give error message
                return
    ...
    gap_label_1.config(text = 'Previous data Entered') #will give confirmation message to user that data has been entered

    try:
        showSection(nextLabel, nextEntry, nextCheck, nextAuto, nextEnter)
        gap_label_1.config(text = listOfEntryCategories[str(entry)])
...
gap_label_1 = Label(my_frame2, bg = 'white')
...
gap_label_1.grid(column = 2, row = 12, sticky = W)
